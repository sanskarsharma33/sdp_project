    ...
    render() {
        ...
        return (
            <div>
                ...
                {this.props.addressList.length===0 ? <div></div>:<center><h2>Select Address</h2></center>}
                <br>
                </br>
                <br/>
                <center>
                <div className="card-group">
                {
                    this.props.addressList.map(item => {
                        return (
                            <div className="card" style={{maxWidth: "15rem", margin:"10px 10px 10px"}}>
                                <div className="card-body">
                                    <h5 className="card-title">{item.address_title}</h5>
                                    <p className="card-text">{item.address}</p>
                                    <p className="card-text">{item.pincode}</p>
                                    <a id={item.id} className="btn btn-primary">Select</a>
                                    <a onClick={this.onAddressDelete} id={item.id} className="btn btn-danger" style={{margin:"2px 2px 2px"}}>Delete</a>
                                </div>
                            </div>
                        )
                    })
...
const mapStateToProps = (state) => ({
  ...
  export default connect(mapStateToProps, { getAddress, deleteAddress, addAddress})(addProduct);
